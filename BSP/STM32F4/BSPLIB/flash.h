...
uint32_t PB_Flash_ReadWord(uint32_t faddr);  //read word (16bits)
void PB_Flash_Write(uint32_t WriteAddr, uint32_t *pBuffer, uint32_t NumToWrite);  //write the wanted length data from assign address
void PB_Flash_Read(uint32_t ReadAddr, uint32_t *pBuffer, uint32_t NumToRead);  //read half word data(16 bits) from assign address
